...
vows.describe('Nestor').addBatch({
    'queue': {
        ...
        'should not log anything when there is nothing in the queue': function (topic) {
            var _path, _method,
                messages = [],
                console = {
                    log: function (message) {
                        assert.fail('Console log should not have been called.');
                    }
                },
                service = {
                    send: function (path, method, successCb, errorCb) {
    ...
    'version': {
        ...
        'should log error message when header x-jenkins does not exist': function (topic) {
            ...
            assert.equal(_method, 'HEAD');
            assert.equal(messages.length, 1);
            assert.equal(messages[0], 'Unknown');
        }
    }
}).export(module);