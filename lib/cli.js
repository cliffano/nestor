...
function __exec(args, cb) {
  ...
  if (url && args.parent && args.parent.interactive) {
    commander.prompt(i18n.__('Username: '), function (user) {
      commander.password(i18n.__('Password: '), function (pass) {
...
function _build(jobName, params, args) {
  ...
  const PENDING = 5000;
  var message = i18n.__('Job %s was started successfully'),
    cb;
  ...
  function execCb(jenkins) {
    jenkins.stop(
      ...
      bag.exitCb(null, function (result) {
        console.log(i18n.__('Job %s was stopped successfully'), jobName);
      })
    ); 
  }
  __exec(args, execCb);
...
function _dashboard(args) {
  function execCb(jenkins) {
    jenkins.dashboard(bag.exitCb(null, function (result) {
      if (result.length === 0) {
        console.log(i18n.__('Jobless Jenkins'));
      } else {
        result.forEach(function (job) {
          var status = i18n.__(job.status),
...
function _discover(host, args) {
  ...
  function execCb(jenkins) {
    jenkins.discover(host, bag.exitCb(null, function (result) {
      console.log(i18n.__('Jenkins ver. %s is running on %s'),
      ...
      if (!_.isEmpty(_.keys(result))) {
        _.keys(result).forEach(function (computer) {
          ...
          result[computer].forEach(function (executor) {
            if (executor.idle) {
              console.log('  - ' + i18n.__('idle'));
            } else {
              console.log('  - %s | %s%%s', executor.name, executor.progress, (executor.stuck) ? i18n.__(' stuck!') : '');
            }
          });
        });
      } else {
        console.log(i18n.__('No executor found'));
      }
    }));
  }
  __exec(args, execCb);
...
function _job(name, args) {
  function execCb(jenkins) {
    jenkins.job(name, bag.exitCb(null, function (result) {
      var status = i18n.__(result.status),
...
function _queue(args) {
  function execCb(jenkins) {
    jenkins.queue(bag.exitCb(null, function (result) {
      if (result.length === 0) {
        console.log(i18n.__('Queue is empty'));
      } else {
        result.forEach(function (job) {
          console.log('- %s', job);
        });
...
function _version(args) {
  function execCb(jenkins) {
    jenkins.version(bag.exitCb(null, function (result) {
      console.log(i18n.__('Jenkins ver. %s', result));
    }));
  }
  __exec(args, execCb);
}
