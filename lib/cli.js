...
function __monitor(notifier, jobName, schedule) {
...
function _ninja(jobName, schedule) {
  ...
  __monitor(ninjaBlocks, jobName, schedule);
}

function _buildLight(jobName, schedule, args) {
  var buildLight = new BuildLight({ scheme: args.scheme.split(','), usbled: args.usbled });
  __monitor(buildLight, jobName, schedule);
}
