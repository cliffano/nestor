...
Bot.prototype.connect = function (host, channel, opts) {
  ...
  console.log(i18n.__('Connecting to %s as %s'), host, this.nick);
  this.client = new irc.Client(host, this.nick);

  this.client.addListener('error', function (err) {
    console.error(i18n.__('Error: '), err);
  });
  ...
  this.client.addListener('message' + this.channel, function (from, message) {
    if (message.match(new RegExp('^' + self.nick))) {
      ...
      if (command === undefined) {
        self.say(i18n.__('Usage: %s <command> <arg1> ... <argN>'), self.nick);
      } else if (self.commands[command]) {
        self.commands[command].apply(self, args);  
      } else {
        self.say(i18n.__('Command \'%s\' is not supported'), command);
      }
    }
  });

  console.log(i18n.__('Joining channel %s'), self.channel);
  setTimeout(function () {
...
Bot.prototype.disconnect = function () {
  if (!this.client) {
    console.error(i18n.__('Not connected yet, bot#connect must be called first'));
  } else {
    console.log(i18n.__('Leaving channel %s and disconnecting'), this.channel);
