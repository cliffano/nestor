...
function build(cb) {
  ...
  return function (args, name, paramsString) {
    ...
    function jenkinsCb(jenkins) {
      ...
      const jenkinsPollRetries = args.poll || 15;

      function streamIfReady(buildUrl, ready, retries) {
        if (ready) {
          _console(passThroughCb)(name, null, args);
        } else {
          if (retries > 0) {
...
function _console(cb) {
  return function (args, name, buildNumber) {
 ...
 */
function copy(cb) {
  return function (args, existingName, newName) {
    function resultCb(result) {
