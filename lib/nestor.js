                ...
                'ok': function () {
                    ...
                    if (jobs && jobs.length > 0) {
                        jobs.forEach(function (job) {
                            that.console.log(that.STATUS[job.color] + '\t' + job.name);
                        });
                    } else {
...
Nestor.prototype.job = function (job) {
    var that = this,
        successCb = function (statusCode, headers, result) {
            var actions = {
                404: function () {
                ...
                'ok': function () {
                    ...
                    that.console.log('Status: ' + that.STATUS[json.color]);
                    json.healthReport.forEach(function (report) {
                        that.console.log(report.description);
                    });
                }
            };
            that._handle(statusCode, actions);
        };
    this.service.send('/job/' + job + '/api/json', 'GET', successCb, this.errorCb);
};
