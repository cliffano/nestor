...
Nestor.prototype._isError = function (statusCode) {
    var isError = false;
    if (statusCode.toString().match(/^(4|5)/)) {
        isError = true;
    }
    return isError;
};
Nestor.prototype.build = function (job, params) {
    var that = this,
        successCb = function (statusCode, headers, result) {
            if (!that._isError(statusCode)) {
                that.console.log(that.PREFIX + 'Job was started successfully');
            } else if (statusCode === 404) {
                that.console.error(that.PREFIX + 'Job does not exist');
            } else {
