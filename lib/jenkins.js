...
function _request(method, url, expectedStatusCodes, resultCb, cb) {
  request({
    method: method,
    uri: url
  }, function (err, result) {
    ...
    } else if (expectedStatusCodes.indexOf(result.statusCode) >= 0) {
      resultCb(result);
    } else {
...
function _success(statusCode) {
  return (statusCode && statusCode.toString()[0] === '2') ? true : false;
}
...
Jenkins.prototype.dashboard = function (cb) {
  _request('get', this.url + '/api/json', [200], function (result) {
