...
function Jenkins(url) {
  ...
  function _authFail(result, cb) {
    cb(new Error(i18n.__('Authentication failed - incorrect username and/or password in Jenkins URL')));
  }

  function _authRequire(result, cb) {
    cb(new Error(i18n.__('Jenkins requires authentication - set username and password in Jenkins URL')));
  }
...
Jenkins.prototype.build = function (jobName, params, cb) {
  ...
  function _notFound(result, cb) {
    cb(new Error(util.format(i18n.__('Job %s does not exist'), jobName)));
  }

  function _paramsRequire(result, cb) {
    cb(new Error(util.format(i18n.__('Job %s requires build parameters'), jobName)));
  }
...
Jenkins.prototype.filteredBuild = function (criteria, cb) {
  ...
  function iterator(item, cb) {
    self.build(item.name, null, function (err, result) {
      if (!err) {
        console.log(i18n.__('Job %s was started successfully'), item.name);
      }
      cb(err);
    });
  }
...
Jenkins.prototype.discover = function (host, cb) {
  ...
  var socket = dgram.createSocket('udp4'),
    buffer = new Buffer(i18n.__('Long live Jenkins!')),
  ...
  setTimeout(function () {
    cb(new Error(util.format(i18n.__('Unable to find any Jenkins instance on %s'), host)));
  }, TIMEOUT);
};
...
Jenkins.prototype.job = function (name, cb) {
  ...
  function _notFound(result, cb) {
    cb(new Error(util.format(i18n.__('Job %s does not exist'), name)));
  }
...
Jenkins.prototype.version = function (cb) {
  ...
  function _success(result, cb) {
    if (result.headers['x-jenkins']) {
      ...
      cb(new Error(i18n.__('Not a Jenkins server')));
    }
  }
